...
class MyPanel(val activity: MainActivity, val list: ArrayList<Obstacle>) : View(activity) {
    ...
    var start = true
    var shootLocate = Vector2f(0f, 0f)
    ...
    override fun onDraw(canvas: Canvas?) {
        if (canvas != null) {
            ...
            //Draw out of sight tips
            showNear(canvas)
            //Draw shoot target
            if (weapon.bulletSpeed != 0f) {
                val enemyLocation = activity.selected.location
                val bulletVelocity = weapon.bulletSpeed
                ...
                val anotherAngle = Math.PI - angle - shootAngle
                /*val absAnotherAngle=when{
                    anotherAngle>Math.PI -> anotherAngle-Math.PI
                    anotherAngle<0 -> anotherAngle+Math.PI
                    else -> anotherAngle
                }*/
                val length = toEnemy.length() / Math.sin(anotherAngle).toFloat() * Math.sin(angle.toDouble()).toFloat()
                val target = toEnemy.normalize() * length
                target.rotateAroundOrigin(shootAngle.toFloat(), false)
                shootLocate = target + activity.transport.location
                paint.color = Color.BLUE
                val drawLactation = shootLocate + axis
                canvas.drawOval(drawLactation.x - 2, drawLactation.y - 2, drawLactation.x + 2, drawLactation.y + 2, paint)
            }
        }
    }
    ...
    enum class Weapon(val value: Int, val bulletSpeed: Float) {
        GUN(0, 400f), MISSILE(1, 0f), RADIUS(2, 1500f), SUPER_GUN(3, 500f);
                ...
                if (weapon.bulletLeft > 0) {
                    when (weapon) {
                        Weapon.GUN -> {
                            ...
                            bullet.velocity = activity.transport.velocity.add(activity.transport.velocity.normalize().mult(weapon.bulletSpeed))
                        ...
                        Weapon.MISSILE -> {
                            ...
                            bullet.velocity = Vector2f(activity.transport.velocity + activity.transport.velocity.normalize() * 200f)
                        ...
                        Weapon.SUPER_GUN -> {
                            val bullet = GunBullet(activity)
                            bullet.velocity = (shootLocate-activity.transport.location).normalize() * weapon.bulletSpeed
