...
class MainActivity : AppCompatActivity() {
    ...
    private var settings: Settings = Settings(1 / 9f, 10, 100, 500, 100, 2000, 200)
    ...
    fun create() {
        ...
        for (key in mapSet.keys) {
            ...
            dialogView.findViewById<Spinner>(key).setSelection(selection)
        }
        dialogView.findViewById<Button>(R.id.finished).setOnClickListener {
            ...
            mapSet=settings.toHashMap()
            val fileStream=FileOutputStream(filesDir.path+"/setting")
            val stream=ObjectOutputStream(fileStream)
            println("mapSet = ${mapSet}")
            stream.writeObject(mapSet)
    ...
    var mapSet = HashMap<Int, Float>()
    fun readMap() {
        if (File(filesDir.path + "/setting").exists()) {
            val fileStream = FileInputStream(filesDir.path + "/setting")
            val os = ObjectInputStream(fileStream)
            mapSet = os.readObject() as HashMap<Int, Float>
            settings = Settings.fromHashMap(mapSet)
        }else{
            mapSet=settings.toHashMap()
        }
    }
